...
def test_paid_mode_blocks_nc_backend_without_deduct(billing_dir, monkeypatch):
    monkeypatch.setenv("MORPHANUS_PAID_MODE", "1")
    monkeypatch.setenv("MORPHANUS_INFERENCE_MODE", "research_nc")
    with pytest.raises(HTTPException) as exc:
        run_swap()
...
def test_paid_mode_allows_commercial_mode_and_deducts(billing_dir, monkeypatch):
    monkeypatch.setenv("MORPHANUS_PAID_MODE", "1")
    monkeypatch.setenv("MORPHANUS_INFERENCE_MODE", "commercial_external")
